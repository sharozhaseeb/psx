...
| 5 | Existing `refresh_announcements` returns 50 rows but `get_announcements` returns `[]` for most symbols. Is the issue (a) symbol-specific (only some have announcements) or (b) a persistence/query bug? | Phase 1 announcement-body fix scope depends on which. | **RESOLVED 2026-05-24 (probe 0.4):** Mostly (a), with a separate Bug A. The `/announcements` POST returns only the latest 50 across all symbols — NETSOL/TRG simply aren't in that window. When a symbol IS present (e.g., AABS), `_get_announcements_impl` returns rows correctly. **Separate Bug A confirmed:** every cached row has `body IS NULL` because the HTML feed has no body column (DATE\|TIME\|SYMBOL\|NAME\|TITLE\|action-links) and `parse_announcements` never sets `body`. To populate bodies, Phase 1 must either (i) fetch the linked PDF per row, or (ii) widen the offset/count POST to pull more pages so per-symbol queries hit. |
| 6 | `search_symbol` returned `[]` for NETSOL/TRG despite being in cache. Is the SQL using `LIKE` correctly, or is the query column missing? | Phase 1 search fix needs to know the actual current behavior. | **RESOLVED 2026-05-24 (probe 0.5):** Currently matches both `symbol` (exact-match score 1.0 or fuzzy * 0.9) and `name` (SequenceMatcher ratio). NETSOL/TRG/SYS/"systems" all return correct results today. Needs to also match `sector`: searching "TECHNOLOGY" returned 1 result (NETSOL, by name overlap) when 7 of the 8 cached symbols share `TECHNOLOGY & COMMUNICATION`. Phase 1 fix: extend `_score(query, ...)` to also score against `sector`. Secondary finding: the cache only holds 8 symbols (those queried in prior sessions) — bulk-populating via `refresh_symbols_from_payload` would dramatically increase recall for any search, but that's a separate concern. |
