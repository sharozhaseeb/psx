...
| 2 | Does the `dps.psx.com.pk/company/<SYM>` payload include a dividend history table? | Determines whether `get_dividend_history` can be added now or deferred. | Re-inspect the existing fixture in Task 0.2 — the data may already be present and unparsed. |
| 3 | Does `dps.psx.com.pk/company/<SYM>` include balance-sheet line items (total assets, current liabilities, LT debt) — not just income-statement? | Piotroski F-Score (next plan) depends on this. Documenting now avoids scoping surprises later. | Inspect fixture in Task 0.2. Out-of-scope for *this* plan either way, but the finding informs the next plan. |
| 4 | Are PSX historical OHLC prices in `dps.psx.com.pk/historical` adjusted for bonus issues and splits, or raw? | Affects validity of 52w high/low and (future) backtest. | Spot-check 1-2 known corporate actions in Task 0.3 against raw prices. |
